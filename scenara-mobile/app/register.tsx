...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    setError("");
    if (!displayName.trim()) { setError(t.auth.displayName + " is required"); return; }
    if (!email.trim()) { setError(t.auth.email + " is required"); return; }
    if (password.length < 6) { setError(t.auth.minPwd); return; }
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: "center", padding: 24 }} keyboardShouldPersistTaps="handled">
          ...
          <View style={{ alignItems: "center", marginBottom: 40 }}>
            ...
            <Text style={{ color: TEXT_MID, fontSize: 14, fontFamily: "DMSans_400Regular", marginTop: 6 }}>{t.auth.startTagline}</Text>
          </View>

          {/* Benefits strip */}
          <View style={{ flexDirection: "row", justifyContent: "center", gap: 0, marginBottom: 24 }}>
            {[
              { icon: "💰", label: "$10,000", sub: "Free Balance" },
              { icon: "🏆", label: "Compete", sub: "Leaderboard" },
              { icon: "🔒", label: "No Risk", sub: "Simulated" },
            ].map((item, i) => (
              <View key={i} style={{ flex: 1, alignItems: "center", paddingVertical: 10, backgroundColor: i === 1 ? "rgba(124,92,252,0.06)" : "transparent", borderRadius: 12, borderWidth: i === 1 ? 1 : 0, borderColor: BORDER_P }}>
                <Text style={{ fontSize: 18, marginBottom: 4 }}>{item.icon}</Text>
                <Text style={{ color: TEXT, fontSize: 13, fontFamily: "DMSans_700Bold" }}>{item.label}</Text>
                <Text style={{ color: TEXT_MID, fontSize: 10, fontFamily: "DMSans_400Regular" }}>{item.sub}</Text>
              </View>
            ))}
          </View>
          ...
          <View style={{ backgroundColor: CARD, borderRadius: 20, padding: 24, borderWidth: 1, borderColor: BORDER_P, maxWidth: 440, width: "100%", alignSelf: "center" }}>
            <Text style={{ color: TEXT, fontSize: 22, fontFamily: "DMSans_700Bold", marginBottom: 6 }}>{t.auth.createAccount}</Text>
            <Text style={{ color: TEXT_MID, fontSize: 14, fontFamily: "DMSans_400Regular", marginBottom: 24 }}>{t.auth.free}</Text>
            ...
            <Text style={{ color: TEXT_SUB, fontSize: 12, fontFamily: "DMSans_700Bold", letterSpacing: 0.8, marginBottom: 8 }}>{t.auth.displayName}</Text>
            <View style={{ backgroundColor: SURFACE, borderRadius: 12, borderWidth: 1, borderColor: BORDER, paddingHorizontal: 14, marginBottom: 16 }}>
              <TextInput value={displayName} onChangeText={setDisplayName} placeholder={t.auth.displayNamePh} placeholderTextColor={TEXT_MID} autoCorrect={false} style={{ color: TEXT, fontSize: 15, fontFamily: "DMSans_400Regular", paddingVertical: 14 }} />
            </View>
            ...
            <View style={{ backgroundColor: SURFACE, borderRadius: 12, borderWidth: 1, borderColor: BORDER, paddingHorizontal: 14, marginBottom: 16 }}>
              <TextInput value={password} onChangeText={setPassword} placeholder={t.auth.minPwd} placeholderTextColor={TEXT_MID} secureTextEntry style={{ color: TEXT, fontSize: 15, fontFamily: "DMSans_400Regular", paddingVertical: 14 }} />
            </View>

            <Text style={{ color: TEXT_SUB, fontSize: 12, fontFamily: "DMSans_700Bold", letterSpacing: 0.8, marginBottom: 8 }}>{t.auth.confirmPwd}</Text>
            <View style={{ backgroundColor: SURFACE, borderRadius: 12, borderWidth: 1, borderColor: BORDER, paddingHorizontal: 14, marginBottom: 24 }}>
              <TextInput value={confirm} onChangeText={setConfirm} placeholder={t.auth.repeatPwd} placeholderTextColor={TEXT_MID} secureTextEntry style={{ color: TEXT, fontSize: 15, fontFamily: "DMSans_400Regular", paddingVertical: 14 }} onSubmitEditing={handleRegister} />
            </View>
            ...
            <View style={{ backgroundColor: "rgba(34,197,94,0.08)", borderWidth: 1, borderColor: "rgba(34,197,94,0.2)", borderRadius: 10, padding: 12, marginBottom: 20, flexDirection: "row", alignItems: "center", gap: 8 }}>
              ...
              <Text style={{ color: GREEN, fontFamily: "DMSans_500Medium", fontSize: 13 }}>{t.auth.startBalance}</Text>
            </View>
            ...
            <TouchableOpacity onPress={handleRegister} disabled={loading} style={{ borderRadius: 14, overflow: "hidden", marginBottom: 16 }}>
              <LinearGradient colors={loading ? ["#1a1a2e", "#1a1a2e"] : [BLUE, PURPLE, PINK]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={{ paddingVertical: 15, alignItems: "center" }}>
                {loading ? <ActivityIndicator color="white" /> : <Text style={{ color: "white", fontFamily: "DMSans_700Bold", fontSize: 15 }}>{t.auth.createBtn}</Text>}
              </LinearGradient>
            </TouchableOpacity>

            <View style={{ flexDirection: "row", justifyContent: "center", alignItems: "center", gap: 4 }}>
              <Text style={{ color: TEXT_MID, fontFamily: "DMSans_400Regular", fontSize: 14 }}>{t.auth.haveAccount}</Text>
              <TouchableOpacity onPress={() => router.push("/login")}>
                <Text style={{ color: PURPLE, fontFamily: "DMSans_700Bold", fontSize: 14 }}>{t.auth.signInBtn}</Text>
              </TouchableOpacity>
            </View>
          </View>
