...
function ResolveModal({ target, onClose, onResolved }: { target: ResolveTarget; onClose(): void; onResolved(): void }) {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      Alert.alert("Resolved", `${r.data.total_winners ?? 0} paid out`, [{ text: "Done", onPress: () => { onResolved(); onClose(); } }]);
...
export default function HomeScreen() {
  ...
  const handlePredict = async (scenarioId: number) => {
    ...
    } catch { setError("Could not place position"); }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: BG }}>
      ...
      <SafeAreaView style={{ flex: 1 }}>
        ...
        {/* Banners */}
        {message ? <View style={{ marginHorizontal: 16, marginTop: 8, backgroundColor: "rgba(34,197,94,0.08)", borderColor: "rgba(34,197,94,0.2)", borderWidth: 1, padding: 9, borderRadius: 10 }}><Text style={{ color: GREEN, fontFamily: "DMSans_500Medium", fontSize: 12 }}>✓  {message}</Text></View> : null}
        {error ? <View style={{ marginHorizontal: 16, marginTop: 8, backgroundColor: "rgba(239,68,68,0.08)", borderColor: "rgba(239,68,68,0.2)", borderWidth: 1, padding: 9, borderRadius: 10 }}><Text style={{ color: RED, fontFamily: "DMSans_500Medium", fontSize: 12 }}>{error}</Text></View> : null}
